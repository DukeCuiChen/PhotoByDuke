...
//			addCookie();
			...
			if ((requestCallback != null)) {
				LogWrapper.logD("duke1");
				if (statusCode == HttpStatus.SC_OK) {
					LogWrapper.logD("duke2");
					// 更新服务器时间和本地时间的差值
					updateDeltaBetweenServerAndClientTime();

					String  data = BaseUtils.inputStream2String(response.getEntity().getContent());
					// 把成功获取到的数据记录到缓存
					if (urlData.getNetType().equals(REQUEST_GET)
...
//					}
			...
			LogWrapper.logD("duke5");
			handleNetworkError("网络异常");
		} catch (final IOException e) {
			LogWrapper.logD("duke6");
			handleNetworkError("网络异常");
		}
	}
